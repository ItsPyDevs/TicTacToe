...
class Game:
    ...
    def welcome_screen(self):
        ...
        Label(self.welcome_frame, text='Choisit le mode de jeu',
                bg='#DCE0E1', fg='#000000',
                font=('',20)).pack(padx=10,pady=40)

        Button(self.welcome_frame, text='Joueur contre Pc',
                bg='#FFF000', fg='#000000',
                command=lambda: self.player_info_screen(1)).pack(padx=30,pady=30)
        
        Button(self.welcome_frame, text='Joueur contre Joueur',
                bg='#FFF000', fg='#000000',
    ...
    def player_info_screen(self,player_number):
        ...
        Label(self.player_info_frame,text='Entre Les info du Joueur',
                bg='#DCE0E1', fg='#000000',
        ...
        Label(self.player_info_frame,text="Appuis sur commencer pour choisir qui commence",
                bg='#DCE0E1', fg='#000000',
                font=('',10)).pack(pady=20)

        self.toss_bt = Button(self.player_info_frame,text='Commencer',command=lambda: self.toss(entryList))
        self.toss_bt.pack()
    ...
    def toss(self,lst):
        ...
        if self.is_p1_turn:
            ...
            Label(self.toss_frame,text=f'{self.p2.name} a Gagné',
                    bg='#DCE0E1', fg='#000000',
                    font=('',10)).pack(pady=10)
            ...
            Label(self.toss_frame,text=f"{self.p2.name} A Gagné et a séléctioné le signe 'O'",
                    bg='#DCE0E1', fg='#000000',
    ...
    def player_vs_computer_rule(self,index):
        ...
        self.l1.config(text=self.p2.name + " a jouer")
        self.count += 1
    ...
    def player_vs_player_rule(self,index):
        if self.is_p1_turn:
            ...
            self.l1.config(text=self.p1.name + " a jouer")
            self.is_p1_turn=True
    ...
    def check_result(self,player):
        for i in range(3):
            ...
            if self.gb_List[i*3]['text'] == self.gb_List[i*3+1]['text'] == self.gb_List[i*3+2]['text'] == player.marker:
                ...
                self.root.after(500,lambda: self.display_massage('Félicitation',f'{player.name} a gagné'))
        ...
        if self.count == 9 and not self.game_over:
            self.root.after(500,lambda: self.display_massage('égalité','Personne a Gagné'))
    ...
    def display_massage(self,massage_header,massage):
        ...
        Button(self.massage_frame,text='Rejouer',bg='#FFF000', fg='#000000',command=self.rematch).pack(pady=5)
        Button(self.massage_frame,text='Retour Menu',bg='#FFF000', fg='#000000',command=self.main_menu).pack(pady=5)
        Button(self.massage_frame,text='Sortir',bg='#FFF000', fg='#000000',command=self.root.destroy).pack(pady=5)
